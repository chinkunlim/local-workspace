...
**Decision**: Migrate all shared utilities (path resolution, state tracking, diff, daemon) from voice-memo-local code into `core/`.
...
- `skills/voice-memo/scripts/utils/subject_manager.py` — stripped to 3 CLI-interaction functions only
...
**Decision**: Remove all `if skill_name == "voice-memo"` / `if skill_name == "pdf-knowledge"` branches from `PathBuilder`. Instead, read `paths.phases` from each skill's `config.yaml`.
