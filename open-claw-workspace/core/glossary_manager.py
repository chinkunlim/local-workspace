...
class GlossaryManager:
    ...
    def sync_all(self, logger=None) -> int:
        """
        Scan voice-memo state directories for glossary.json files.
        ...
        if not os.path.exists(self.priority_terms_path):
            if logger: logger.warning(f"找不到全域字彙庫路徑: {self.priority_terms_path}")
            return 0
            
        with open(self.priority_terms_path, "r", encoding="utf-8") as f:
            try:
                priority_data = json.load(f)
            except json.JSONDecodeError:
                if logger: logger.error("全域字彙庫格式破裂。")
                return 0
        ...
        for gfile in glob.glob(search_pattern):
            try:
                ...
                if logger: logger.warning(f"讀取地方字彙庫失敗 ({gfile}): {e}")
        ...
        if added_count > 0:
            ...
            if logger: logger.info(f"🔄 跨模組術語同步完成！成功引入 {added_count} 個新詞彙至中央防護清單。")
    ...
    def get_global_prompt_injection(self) -> str:
        ...
        terms_str = "、".join([t for t in protection_list if not t.startswith("_")]) # Filter out _comment
