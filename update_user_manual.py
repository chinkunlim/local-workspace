...
content = """# Open Claw Knowledge Ecosystem: Comprehensive Operator's Manual
...
1. **Unified Dashboard**: Every module renders a standardized `📊 [Skill Name] State & DAG Tracking Panel` in the terminal for consistent UX.
2. **Graceful Interruptions & Notifications**: Press `Ctrl+C` to halt operations. The system intercepts `KeyboardInterrupt`, saves state checkpoints safely, and dispatches a macOS native alert (`Execution Interrupted`). Upon successful completion, a `Pipeline Execution Complete` notification is dispatched. **No additional software installation is required.**
"""

with open("docs/USER_MANUAL.md", "w", encoding="utf-8") as f:
    f.write(content)

print("Updated docs/USER_MANUAL.md")
