...
class VerificationGateHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/":
            ...
            self.end_headers()
            html = self.server.gate.generate_html()
...
class VerificationGate:
    ...
    def _parse_original_text_to_html(self, text: str) -> str:
        """
        Parses custom [? word | timestamp ?] tags into clickable spans.
        """
        # Escape HTML first
        import html
        ...
        def repl(match):
            word = match.group(1)
            time = match.group(2)
            return f'<span class="uncertain" onclick="playAudio({time})">{word}</span>'
    ...
    def generate_html(self) -> str:
        ...
        right_html = html.escape(self.llm_text) if "html" in globals() else self.llm_text.replace("<", "&lt;").replace(">", "&gt;")
    ...
    def start(self) -> str:
        """
        Starts the ephemeral WebUI, blocks until the user submits, and returns the final text.
        """
        import html # Ensure loaded
        
        server = HTTPServer(("localhost", self.port), VerificationGateHandler)
        server.gate = self
        
        # Use rich to print a nice box if available, otherwise fallback to print
        msg = f"  👉 ACTION REQUIRED: Open http://localhost:{self.port} to verify output."
        try:
            from rich.console import Console
            from rich.panel import Panel
            Console().print(Panel(msg, title="[bold yellow]Verification Gate Paused", border_style="yellow"))
        except ImportError:
            print("\n" + "="*60)
        ...
        # This will block until server.shutdown() is called in the POST handler
