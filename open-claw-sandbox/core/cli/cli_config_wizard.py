...
def prompt_choice(phase_key, profiles, current_active):
    ...
    while True:
        try:
            choice = input(
                f"\n👉 請輸入欲使用的組態編號 (直接按 Enter 保留目前設定 [{current_active}]): "
            ).strip()
            if not choice:
...
def main():
    ...
    parser.add_argument(
        "--skill", required=True, help="Target skill name (e.g. audio-transcriber, doc-parser)"
    )
    ...
    json_config = os.path.join(
        pb.canonical_dirs.get("input", pb.base_dir), "config.json"
    )  # fallback for older voice memo
