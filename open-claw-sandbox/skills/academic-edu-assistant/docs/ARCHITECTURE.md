# academic-edu-assistant — Architecture

## Role

`academic-edu-assistant` is a **RAG-powered, cross-document comparison and flashcard generation skill**.
...
2. **Multi-document Input**: Unlike other skills, does not operate on a single file — it scans all Markdown files in `data/academic-edu-assistant/input/<subject>/`.
...
```
Input: data/academic-edu-assistant/input/<subject>/*.md
  │
  ...
  │     └── Output: data/academic-edu-assistant/output/01_comparison/<subject>/report.md
  │
  └── Phase 2: Anki Flashcard Generation (p02_anki.py)
        ├── Read comparison report (or raw input files)
        ├── Extract Q&A pairs via LLM
        └── Output: data/academic-edu-assistant/output/02_anki/<subject>/cards.csv
```

## Directory Structure

```
skills/academic-edu-assistant/
