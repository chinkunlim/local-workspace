...
class Phase1Compare(PipelineBase):
    ...
    def run(self, force=False, subject=None, file_filter=None, single_mode=False, query=None, resume_from=None):
        self.info(f"✨ 啟動 Phase 1：RAG 交叉比對")
        ...
        db_path = os.path.abspath(os.path.join(self.base_dir, "..", "telegram-kb-agent", "state", "chroma_db"))
...
"""
        ...
        with self.state_manager._lock:
            ...
            if f"{safe_name}.md" not in self.state_manager.state["Queries"]:
                self.state_manager.state["Queries"][f"{safe_name}.md"] = {p: "⏳" for p in self.state_manager.PHASES}
