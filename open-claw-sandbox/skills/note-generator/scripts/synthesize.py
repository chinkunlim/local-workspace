...
class NoteGenerator(PipelineBase):
    ...
    def _agentic_mermaid_retry(self, node_text: str, base_prompt: str, model: str, options: dict, max_retries: int) -> str:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            error_str = "\n".join(errors)
            self.warning(f"⚠️ [Agentic Retry {attempt}/{max_retries}] 偵測到 Mermaid 語法錯誤: {error_str}")
    ...
    def _synthesize_chunked(self, content: str, map_tpl: str, reduce_tpl: str, model: str, options: dict, label: str, map_size: int):
    ...
    def run(self, markdown_text: str, subject: str = "Default", label: str = "document", figure_list: str = "", glossary_injection: str = "") -> str:
        """
        ...
        """
        self.info(f"✨ [NoteGenerator] 啟動知識合成: [{subject}] {label} ({len(markdown_text):,} 字元)")
        
        config = self.get_config("synthesize", subject_name=subject)
        
        # Override active profile if specified
        if self.profile_override:
            profile_data = self.config_manager.get_section("synthesize", {}).get("profiles", {}).get(self.profile_override)
            if profile_data:
        ...
        reduce_tpl = reduce_tpl.replace("{GLOSSARY}", glossary_injection).replace("{FIGURES}", figure_list)
        map_tpl = map_tpl.replace("{GLOSSARY}", glossary_injection).replace("{FIGURES}", figure_list)

        try:
            note_tag = None
            if len(markdown_text) > chunk_thresh:
                res, sc, tc = self._synthesize_chunked(markdown_text, map_tpl, reduce_tpl, model, options, label, map_size)
                yaml_mr = "true"
                yaml_chk = f"{sc}/{tc}"
                if sc < tc:
                ...
                pmpt = reduce_tpl.replace("{INPUT_CONTENT}", markdown_text) if "{INPUT_CONTENT}" in reduce_tpl else f"{reduce_tpl}\n\n<transcript>\n{markdown_text}\n</transcript>"
            ...
            res, mm_tag = self._agentic_mermaid_retry(res, reduce_tpl, model, options, mermaid_retry_max)
            if mm_tag and not note_tag: note_tag = mm_tag

            # Content-Loss Guard
            retention_ratio = len(res) / max(1, len(markdown_text))
            self.info(f"📊 [NoteGenerator] 壓縮率檢核: 最終 {len(res):,} 字 / 原始 {len(markdown_text):,} 字 (保留率 {retention_ratio:.1%})")
            
            if retention_ratio < min_retention_ratio:
                raise ValueError(f"[Content-loss Guard] 筆記保留率 {retention_ratio:.1%} 低於下限 {min_retention_ratio:.1%}！")
...
if __name__ == "__main__":
    ...
    if not sys.stdin.isatty():
        ...
        if input_text:
            result = NoteGenerator(profile=args.profile).run(input_text, subject=args.subject, label=args.label)
            print(result)
    else:
        print("Please pipe markdown text via stdin. Example: cat doc.md | python synthesize.py")
