...
data/raw/<subject>/lecture.m4a
          ...
          │
data/audio_transcriber/output/01_transcript/<subject>/lecture.md
          │
          ▼ P2: LLM proofreading + glossary term protection
data/audio_transcriber/output/02_proofread/<subject>/lecture.md
          │
          ▼ P3: Cross-segment merge and refinement
data/audio_transcriber/output/03_merged/<subject>/lecture.md
          │
...
class Phase2Proofread(PipelineBase):
    def __init__(self) -> None:
        ...
        # self.dirs["p2"] → data/audio_transcriber/output/02_proofread (from config.yaml)
...
```yaml
# skills/audio_transcriber/config/config.yaml
paths:
...
- `core.state.state_manager.StateManager` manages `data/audio_transcriber/state/.pipeline_state.json`
...
- `data/audio_transcriber/state/checklist.md` is auto-generated as a human-readable progress tracker
...
data/raw/<subject>/         ──(Watchdog)──►  InboxDaemon
                             ...
                             (Glossary)     (Transcribe)   (Proofread)
                                               ...
                                               (Merge)
                                                  │
                                    data/audio_transcriber/output/03_merged/
```
...
| `core.utils.glossary_manager.GlossaryManager` | Glossary sync with doc_parser |
...
python3 skills/audio_transcriber/scripts/run_all.py --subject YourSubject
...
python3 core/cli/cli_config_wizard.py --skill audio_transcriber
```
...
Call `python3 core/cli/cli_config_wizard.py --skill audio_transcriber` to switch profiles interactively.
