...
class Phase2Proofread(PipelineBase):
    def __init__(self):
        ...
        if lookback is None:
            raise RuntimeError("audio_transcriber context.phase2_lookback_chars is missing")
        self.LOOKBACK_CHARS = int(lookback)
        threshold = self.config_manager.get_nested("thresholds", "phase2_verbatim")
        if threshold is None:
            raise RuntimeError("audio_transcriber thresholds.phase2_verbatim is missing")
