...
class Phase3Merge(PipelineBase):
    def __init__(self):
        ...
        if threshold is None:
            raise RuntimeError("audio_transcriber thresholds.phase3_verbatim is missing")
        self.P3_VERBATIM_THRESHOLD = float(threshold)
        lookback = self.config_manager.get_nested("context", "phase3_lookback_chars")
        if lookback is None:
            raise RuntimeError("audio_transcriber context.phase3_lookback_chars is missing")
