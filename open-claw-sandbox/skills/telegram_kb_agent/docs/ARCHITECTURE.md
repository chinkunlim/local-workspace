# telegram_kb_agent — Architecture

## Role

`telegram_kb_agent` is the **mobile knowledge retrieval interface**. It provides two core services:
...
Indexer (one-shot, run after wiki updates):
  ...
  └── Chunk documents (smart_split)
        └── LLM Embedding → ChromaDB upsert
              └── data/doc_parser/output/vector_db/ (persistent index)
