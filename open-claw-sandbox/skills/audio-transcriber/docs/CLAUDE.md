...
The audio-transcriber skill converts `.m4a` lecture recordings into Obsidian-ready structured knowledge documents through a 5-phase AI pipeline. It is the original and most mature skill in Open Claw.
...
5. **Core imports must use sub-package paths** — see `CODING_GUIDELINES_FINAL.md §3.1`.
...
**Switching models**: Edit `config.yaml` → `phaseN.active_profile` or run `python3 core/cli/cli_config_wizard.py --skill audio-transcriber`
