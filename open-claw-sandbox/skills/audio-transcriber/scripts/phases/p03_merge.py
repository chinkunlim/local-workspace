...
class Phase3Merge(PipelineBase):
    def __init__(self):
        ...
        if threshold is None:
            raise RuntimeError("audio-transcriber thresholds.phase3_verbatim is missing")
        self.P3_VERBATIM_THRESHOLD = float(threshold)
        lookback = self.config_manager.get_nested("context", "phase3_lookback_chars")
        if lookback is None:
            raise RuntimeError("audio-transcriber context.phase3_lookback_chars is missing")
            ...
            for (subj, base_name), tasks_in_group in groups.items():
                ...
                for task in tasks_in_group:
                    ...
                    proof_path = os.path.join(
                        self.dirs["p2"], subj, f"{os.path.splitext(fname)[0]}.md"
                    )
                ...
                for c_idx, chunk in enumerate(chunks):
                    ...
                    try:
                        ...
                        if "---" in res_stripped and header in res_stripped:
                            ...
                            if raw_log.startswith(header):
                                raw_log = raw_log[len(header) :].strip()
                        ...
                        if len(body) < len(chunk) * self.P3_VERBATIM_THRESHOLD:
                            self.log(
                                f"⚠️ 片段 {c_idx + 1} [Phase 3 逐字稿守衛]: 輸出過短，回退至原文",
                                "warn",
                            )
                            body = chunk
