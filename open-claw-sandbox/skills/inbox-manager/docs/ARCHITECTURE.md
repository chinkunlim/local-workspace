# inbox-manager — Architecture

## Role

`inbox-manager` is a **CLI utility skill** for managing the PDF routing rules in
...
| Mode | Behaviour |
|:---|:---|
| `audio_ref` | PDF is linked into `audio-transcriber/input/` as a proofreading reference. No standalone parsing. |
| `doc_parser` | PDF is sent to `doc-parser/input/` for full Markdown extraction. |
...
python3 skills/inbox-manager/scripts/query.py \
