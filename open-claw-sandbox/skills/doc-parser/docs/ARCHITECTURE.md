...
- `core.state.state_manager.StateManager(skill_name="doc-parser")` manages 5-phase progress
- Phase set: `["p0a", "p1a", "p1b", "p1c", "p1d"]`
- `data/doc-parser/state/checklist.md` tracks processing status for every PDF
...
core/config/global.yaml                     ← Hardware thresholds + Ollama runtime
    ↓ deep-merge
skills/doc-parser/config/config.yaml        ← Skill-specific overrides
```
...
| `core.utils.glossary_manager.GlossaryManager` | Syncs terminology from audio-transcriber |
...
python3 core/cli/cli_config_wizard.py --skill doc-parser
```
