...
> Technical decision log for the `inbox_manager` skill.

---

## 2026-04-19 — inbox_manager as a Standalone Skill (not a core/ utility)

**Decision**: Implement inbox routing rule management as a separate `skills/inbox_manager/`
...
**Chosen approach**: `inbox_manager` is a stateless CLI skill that reads and writes
...
**Chosen approach**: `inbox_daemon.py` branches on these exact string values. The `inbox_manager`
