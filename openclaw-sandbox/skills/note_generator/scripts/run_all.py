"""
run_all.py — NoteGenerator Skill Orchestrator (V3.0)
=====================================================
...
Modes:
  1. Batch mode: --subject / --file — processes 1. 掃描 Inbox 目錄:
...
class PhaseNoteGenerator(PipelineBase):
    ...
    def _process_file(self, idx: int, task: dict, total: int) -> Optional[bool]:
        ...
        label = filename.replace("_proofread.md", "")
        if label.endswith(".md"):
            label = label[:-3]

        out_name = f"{label}_notes.md"
        output_path = os.path.join(self.dirs["processed"], subject, out_name)

        self.info(f"🚀 [Phase N1] 合成: [{subject}] {filename}")
        ...
        try:
            result = self.run_single(text, subject=subject, label=label)
    ...
    def run_single(
        ...
        try:
            ...
            yaml_header = (
                ...
                f"pipeline_version: v3.0.0-orchestrator\n"
                f"---\n\n"
            )
...
class NoteGeneratorOrchestrator(PipelineBase):
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        try:
            print(f"\n{'=' * 50}")
            print("🚀 開始執行知識合成 (Phase N1)...")
            print(f"{'=' * 50}")

            p_obj = PhaseNoteGenerator(profile=getattr(args, "profile", None))
...
def main() -> None:
    parser = build_skill_parser(
        "V3.0 Note Generator Pipeline — 知識合成筆記",
