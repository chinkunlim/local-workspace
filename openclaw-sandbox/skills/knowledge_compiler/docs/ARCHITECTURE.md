# knowledge_compiler — Architecture

## Role

`knowledge_compiler` is the **knowledge publishing and bidirectional linking engine**.
It scans the final outputs of factory skills (`audio_transcriber`, `doc_parser`) and
...
1. **Publisher, not producer**: `knowledge_compiler` reads from other skills' outputs. It does NOT re-run or duplicate any processing.
...
```
Input sources:
  ├── data/audio_transcriber/output/03_merged/<subject>/*.md
  └── data/doc_parser/output/05_Final_Knowledge/<subject>/<pdf_id>/content.md
  │
