# SKILL_RULE.md — Telegram KB Agent Skill
...
`telegram_kb_agent` is the user's mobile knowledge retrieval interface. It indexes `data/wiki/`
...
3. **Rebuild index after wiki updates**: The ChromaDB index is NOT automatically updated when `knowledge_compiler` publishes new notes. Operators must manually run `indexer.py` after compilation.
...
| Item | Path |
|:---|:---|
| Bot daemon | `skills/telegram_kb_agent/scripts/bot_daemon.py` |
| Indexer | `skills/telegram_kb_agent/scripts/indexer.py` |
| CLI query tool | `skills/telegram_kb_agent/scripts/query.py` |
| Architecture doc | `skills/telegram_kb_agent/docs/ARCHITECTURE.md` |
...
python3 skills/telegram_kb_agent/scripts/query.py --query "What is Working Memory?"
```
...
rm -f data/telegram_kb_agent/.bot.pid
python3 skills/telegram_kb_agent/scripts/bot_daemon.py
```
